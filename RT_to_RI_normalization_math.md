...
> **[2026-08-24, v3.3] 錨點數不再固定是 6。** 本文件通篇以六點（`x_1…x_6`）舉例，
> 因為當時唯一的來源是 STD 的六個酮。v3.3 新增了第二個來源——資料夾 `.gasprj` 內建的
> `RI_Normalization` 表——**實測 182 個錨點**。
>
> **數學完全不變**：分段線性內插對錨點數量沒有假設，`make_rt_to_ri()` 一行都不用改。
> 下面的檢查清單把 `[x_1, x_6]` 讀成 `[x_first, x_last]` 即可。唯一要留意的是
> `.gasprj` 的表是 VOCal **重採樣後**的等距格點（`log10(RT)` 每 0.01 一點），不是實測
> 錨點，所以「錨點很密」不代表「校正很準」——密度來自重採樣，不是來自更多標準品。

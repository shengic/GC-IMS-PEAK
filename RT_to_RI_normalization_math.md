...
錨點 RT 為本專案已確認的真實數字，RI 用 400/500/600/700/800/900 純示範算術：
...
**這組 $(450 \to 644.1)$ 可以當「內插演算法」的單元測試 ground truth，但不代表校準已驗證**——這裡的 RI 值（400/500/600/700/800/900）是純示範算術，不是本專案真實的甲基酮校準值（真實值仍待補，見 workflow md 第 13 點）。這個測試只驗證 `log10` 轉換、區間搜尋、加權內插三步驟有沒有寫對，測試命名跟註解要明確標示這一點，不要讓人誤以為校準結果已經驗證過（見 workflow md draft.21 第 16 點）。

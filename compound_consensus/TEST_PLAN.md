...
```bash
.venv/Scripts/python.exe -m pytest -q            # 全套 464 項
.venv/Scripts/python.exe -m pytest test3/ -q     # 只跑第三支（226 項）
...
## 1. 進入點與三支並存 — `test3/test_main3.py`（18 項）
...
## 3. 共識邏輯 — `test3/test_consensus.py`（43 項）
...
## 3.5 逐檔規則 — `test3/test_rules_store.py`（16）＋ `test3/test_rules_per_file.py`（22）
...
## 4. 快取與預處理 — `test3/test_logic.py`（11）＋ `test3/test_preprocess.py`（12）
...
| 9 | 模式 2 選同標本的一組 | 勾選同步到左側 `組 ✓` |
| 10 | 按 `3. Consolidate`(門檻 1/2) | 自動切到模式 3;依票數排序 |
